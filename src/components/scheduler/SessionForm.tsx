...
const SessionForm: React.FC = () => {
    ...
    useEffect(() => {
        if (deviceType === "Dwarf II") {
            setExposureOptions(
                [...allowed_exposures.map((e) => e.name)].reverse()
            );
            ...
            setExposureOptions(
                [...allowed_exposuresD3.map((e) => e.name)].reverse()
            );
    ...
    const calculateEndTime = (): { endDate: string; endTime: string } | null => {
        try {
            ...
            const endDateTime = new Date(startDateTime.getTime() + totalExposureTime * 1000);
            return {
    ...
    const generateJsonPreview = (): any => {
        ...
        const data = {
            command: {
                ...
                setup_camera: {
                    do_action:
                        cameraType === "Tele Camera" && checkInteger(count) !== 0,
                    exposure,
                    gain,
                    binning: "0",
    ...
    const saveToJson = () => {
        ...
        if (checkInteger(count) === 0) {
            if (!window.confirm("Count value is 0, so no imaging will take place. Proceed?"))
                return;
        }
            ...
            if (typeof text === "string") {
                try {
                    ...
                    for (let i = 1; i < rows.length; i++) {
                        ...
                        let raVal = "";
                        let decVal = "";
                        if (header.includes("Pane") && header.includes("RA") && header.includes("DEC")) {
                            descriptionVal = `Observation of Mosaic ${row[header.indexOf("Pane")]}`;
                            ...
                            descriptionVal = `Mosaic of ${row[header.indexOf("Catalogue Entry")]}`;
    ...
    return (
        <div className="session-form">
            ...
            <div className="form-group">
                <button className="primary-button" onClick={saveToJson}>Save</button>
            </div>

            <div className="form-group">
                <label className="form-title">Import Telescopius Mosaic CSV</label>
                <input type="file" accept=".csv" className="file-input" onChange={handleCsvImport} />
            </div>
            {showPreview && (
                <div className="modal">

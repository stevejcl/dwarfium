...
function DwarfiumMosaicEngine(target, params, target_div, day_div, year_div, radec_div)
{
    console.log('DwarfiumMosaicEngine');

    var engine_params = {
        fov_x : params.fov_x,
        fov_y : params.fov_y,
        am_fov_x : getDwarfiumMosaicFov(params.fov_x),
        am_fov_y : getDwarfiumMosaicFov(params.fov_y),
    ...
    if (engine_params.horizonSoft != null && engine_params.horizonHard == null) {
        ...
        console.log('DwarfiumMosaicEngine, use soft horizon as hard horizon');
    ...
    return StartDwarfiumMosaicViewerEngine(
                "embedded", 
                target, 
                engine_params, 
                engine_panels, 
...
/* 
 * Utility routine to convert FoV degrees to format used by StartDwarfiumMosaicViewerEngine.
 */
function getDwarfiumMosaicFov(fov)
{
...
function StartDwarfiumMosaicViewerEngine(
    engine_view_type, 
    target, 
    engine_params, 
    engine_panels, 
    ...
    json_target)
{
    ...
    console.log('StartDwarfiumMosaicViewerEngine', engine_view_type, target);
    ...
    console.log('StartDwarfiumMosaicViewerEngine', 'c', c, 'c2', c2);
    if (json_target) {
        EngineViewTargetPanels(json_target);
    } else if (c == '{') {
        console.log('StartDwarfiumMosaicViewerEngine: json target ', image_target);
    ...
    function findCatalogName(name)
    {
            ...
            || upname.substring(0, 7) == 'MESSIER') 
        {
            ...
            retname = upname.replace("MESSIER", "M ").replace(/ {2}/g, " ");
            retname = retname.replace("M", "M ").replace(/ {2}/g, " ");
        } else if (upname.substring(0, 3) == 'NGC') {
            retname = upname.replace("NGC", "NGC ").replace(/ {2}/g, " ");
            catalog_name = 'NGC';
        } else if (upname.substring(0, 2) == 'IC') {
            retname = upname.replace("IC", "IC ").replace(/ {2}/g, " ");
            catalog_name = 'IC';
        } else if (upname.substring(0, 3) == 'RCW') {
            retname = upname.replace("RCW", "RCW ").replace(/ {2}/g, " ");
            catalog_name = 'RCW';
                    ...
                    || upname.substring(0, 7) == 'Barnard') 
        {
            ...
            retname = upname.replace("Barnard", "B ").replace(/ {2}/g, " ");
            retname = retname.replace("B", "B ").replace(/ {2}/g, " ");
        } else if ((upname.substring(0, 3) == 'Ced' && isNumber(upname.substring(3, 4)))
                   || upname.substring(0, 9) == 'Cederblad')
        {
            catalog_name = 'Cederblad';
            retname = upname.replace("Cederblad", "Ced ").replace(/ {2}/g, " ");
            retname = retname.replace("Ced", "Ced ").replace(/ {2}/g, " ");
        } else {
            target_is_catalog_name = false;
    ...
    function EngineInitAladin(aladin_fov, aladin_target)
    {
        ...
        if (aladin && engine_view_type == "all") {
            ...
            console.log("ObjectClicked event listener toegevoegd!");
            aladin.on('objectClicked', function (object) {
                if (object && object.data) {
                    console.log("Intercept objectClicked:", object);

                    // Haal de naam van het object op
                    let rawTargetName = object.data.name || object.data.o || "Unknown";

                    // **Voorkom dubbele encoding**
                    let fixedTargetName = decodeURIComponent(decodeURIComponent(rawTargetName));

                    // **Zet de correcte URL samen zonder Aladin's encoding**
                    let correctUrl = `http://cdsportal.u-strasbg.fr/?target=${encodeURIComponent(fixedTargetName)}`;

                    console.log("Corrigeerde URL:", correctUrl);

                    // **Open de correcte URL zelf in een nieuw tabblad**
                    window.open(correctUrl, "_blank");

                    // **Voorkom dat Aladin zelf een foute URL opent**
                    if (object.action) {
                        object.action = function () { return false; };
                    }
                }
            });
            setTimeout(() => {
                console.log("Intercept Aladin CDS Portal links...");

                document.addEventListener("click", function (event) {
                    if (event.target.tagName === "A" && event.target.href.includes("cdsportal.u-strasbg.fr")) {
                        event.preventDefault(); // Blokkeer de foutieve link

                        // Haal de foutieve URL op
                        let wrongUrl = event.target.href;
                        console.log("Foute URL gedetecteerd:", wrongUrl);

                        // Extraheer de foutieve target uit de URL
                        let urlParams = new URLSearchParams(new URL(wrongUrl).search);
                        let rawTarget = urlParams.get("target");

                        if (rawTarget) {
                            // **Voorkom dubbele encoding**
                            let fixedTarget = decodeURIComponent(decodeURIComponent(rawTarget));
                            console.log("Gezuiverde target:", fixedTarget);

                            // **Bouw de correcte URL**
                            let correctUrl = `http://cdsportal.u-strasbg.fr/?target=${encodeURIComponent(fixedTarget)}`;
                            console.log("Corrigeerde URL:", correctUrl);

                            // **Open de correcte URL in een nieuw tabblad**
                            window.open(correctUrl, "_blank");
                        }
                    }
                }, true);
            }, 3000); // Wacht even zodat de Aladin popup tijd heeft om te laden
    ...
    function createMosaicTableElement(txt)
    {
        ...
        tabdata.style.border = "1px solid #ff0000";
